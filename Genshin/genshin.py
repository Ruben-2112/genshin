...
def Battle_pass_important_dates():
    '''
    It returns the first 5 fridays of every Genshin Impact version
    '''
...
def important_dates(what):
    '''
    It returns a list of important dates depending on what are you refering

    what = string (new_version, banner_reset, Battle_Pass, bilibili)
    '''
...
def datesMatrix(final_day,Currently_primos,Battle_Pass=True,Battle_Pass_Premium=True,events=True):
    '''
    It calculates the primogems that can be achieved from today to a certain final_day

    final_day: datetime format
    Currently_primos: string
    Battle_Pass: Bool
